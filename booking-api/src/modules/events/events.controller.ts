...
import { EventResponseDto, EventsQueryDto } from './dtos';
...
export class EventsController {
  ...
  @ApiQuery({ name: 'search', required: false, type: String, example: 'conference' })
  @ApiQuery({ name: 'sortBy', required: false, enum: ['date', 'price', 'title'], example: 'date' })
  @ApiQuery({ name: 'sortOrder', required: false, enum: ['asc', 'desc'], example: 'asc' })
  async findAll(@Query() query: EventsQueryDto): Promise<EventResponseDto> {
